...
def test_vq_st_gradient1():
    ...
    (grad_inputs,) = torch.autograd.grad(codes, inputs, grad_outputs=[grad_output])
...
def test_vq_st_gradient2():
    ...
    indices = vq(inputs, codebook)
    codes_torch = torch.embedding(
        codebook, indices, padding_idx=-1, scale_grad_by_freq=False, sparse=False
    )

    grad_output = torch.rand((2, 3, 5, 7), dtype=torch.float32)
    (grad_codebook,) = torch.autograd.grad(codes, codebook, grad_outputs=[grad_output])
    (grad_codebook_torch,) = torch.autograd.grad(
        codes_torch, codebook, grad_outputs=[grad_output]
    )
