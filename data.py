...
class MiniImagenet(data.Dataset):
    ...
    splits = {"train": "train.csv", "valid": "val.csv", "test": "test.csv"}
    ...
    def __init__(
        ...
        if not ((train ^ valid ^ test) ^ (train & valid & test)):
            raise ValueError(
                "One and only one of `train`, `valid` or `test` "
                "must be True (train={0}, valid={1}, test={2}).".format(
                    train, valid, test
                )
            )
    ...
    def _fit_label_encoding(self):
        ...
        self._label_encoder = dict(
            (label, idx) for (idx, label) in enumerate(unique_labels)
        )

    def _check_exists(self):
        return os.path.exists(self.image_folder) and os.path.exists(self.split_filename)
...
class TinyImagenet(data.Dataset):
    def __init__(
        ...
        if not ((train ^ valid) ^ (train & valid)):
            raise ValueError(
                "One and only one of `train` or `valid` "
                f"must be True (train={train}, valid={valid})."
            )
        ...
        if download:
            ...
            self.dataset = load_dataset(
                "zh-plus/tiny-imagenet", split=split, cache_dir=root
            )
